...
- `docs/redesign/occupancy-planning-contract.md`, its executable contract, verifier, and `/design-lab/occupancy` close the reversible Wave 5 foundation across roster distinction, capacity sources, overbooking, blocks, requests, expiring holds, future places, funding-hour separation, privacy, and stale-source recovery.
- `docs/redesign/staffing-rota-contract.md`, its executable contract, verifier, and `/design-lab/staffing` close the reversible J05/Wave 3 foundation across source completeness, overlap, presence, effective qualification, absence, target/source cover consequence, breaks, privacy, and stale-source recovery.
- `docs/redesign/daily-care-contract.md`, its executable contract, verifier, and `/design-lab/daily-care` close the reversible J03/Wave 2 foundation across unset observations, shared provenance, exceptions, versioned drafts, sync, atomic submission, parent delivery, privacy, and correction.
- `docs/redesign/communication-contract.md`, its executable contract, verifier, and `/design-lab/communication` close the reversible Wave 4 foundation across source-linked drafts, explicit exclusions, frozen audiences, isolated family conversations, channel receipts, reply obligations, archive, privacy, and correction.
- `docs/redesign/academic-rollover-contract.md`, its executable contract, verifier, and `/design-lab/academic-rollover` close the reversible Wave 6 foundation across source locks, dry-run blockers/impact, immutable backup, isolated restore proof, distinct approvals, locked execution, post-cutover validation, rollback, audit, privacy, and idempotency.
...
The Mobbin MCP is registered, authenticated, and available in this task. Two flow studies and the direction stress test now cover Revolut, Notion, Cursor, Cosmos, Vercel, Duolingo, Duolingo ABC, Genie, 7shifts, Headspace, Things, Flighty, Airbnb, Stripe, and Apple Reminders. The evaluation directly inspects current Headspace, Duolingo, Revolut, Notion, Vercel, and Cosmos screens. One exact Anything screen was retained; exact multi-step Anything, Brightwheel, and Famly results were unavailable, and unrelated matches were excluded. The controlled finalist proof applies Kinetic Kindness and Living Record to identical source-backed operations, incident, and parent-delivery surfaces; the identity-world proof keeps the approved logo seed, care meanings, application jobs, and decision criteria constant while comparing memory asset, voice, image grammar, motion, and cross-surface expression.
...
### 2026-07-11 - Wave 5 accountable occupancy and future-place foundation

- **Question:** Can Kiddz answer live occupancy, booked utilization, and future sellable places without relabeling current class assignment or active-child counts, and can one place move safely from request to hold to booking while funding and billing remain distinct?
- **Evidence:** `Class.maxStudents/capacity`, `Child.classId/enrollmentDate/isActive/isDraft`, branch compliance declared totals, class and branch `_count.children`, active-child dashboard grouping, `t_class.max_students` migration, deliberately ignored legacy `current_students`, absent booking/session/waitlist/hold/block/funding models, competitor source-chain synthesis, Wave 5 plan, jurisdiction and operational architecture.
- **Decision:** Classify current counts only as roster evidence. Calculate effective session capacity from source-revisioned physical, policy, and staffing limits minus active named blocks; keep overbooking explicit; let confirmed bookings and unexpired holds consume separate places; require current request/hold/room/plan revisions for confirmation; atomically create separate attendance-expectation and billing-input identities; keep booked, attended, claimed, and invoiced minutes independent; mask admissions identity and hour values by capability.
- **Artifacts:** `src/lib/redesign-occupancy-planning-contracts.ts`, `src/scripts/verify-redesign-occupancy-planning-contracts.ts`, `/design-lab/occupancy`, `docs/redesign/occupancy-planning-contract.md`, and linked master-plan, architecture, and decision-log updates.
- **Verification:** Deterministic checks prove source-gap withholding, exact capacity arithmetic, explicit conflict places, roster-only classification, live unknown without attendance, block release history, request-without-consumption, hold consumption, expiry return, renewal history, atomic confirmation/downstream projection, stale-source rejection/refresh, exact idempotency, changed-input rejection, capability denial, admissions privacy, and four separate hour ledgers. Agent Browser covers nine states across manager `1440 x 900`, admissions `768 x 1024`, and practitioner `390 x 844` for 27 combinations; each has its expected status, one H1, Meadow live state `Unknown`, zero overflow, unnamed or undersized controls, axe violations, or incomplete findings. Practitioner runs expose no Haddad/Lea identity or hour values. Three capacity-resolution and three admissions transitions update the live region and return focus; expiry renewal retains old history, stale-source refresh restores the held decision, all nine mobile states retain the complete capacity equation, visual scrolling confirms coherent lower sections, and browser logs are empty. Focused ESLint, full TypeScript, all 15 redesign suites, 15 class/branch/child/attendance/accounting parity suites, diff hygiene, and the production build pass; the route verifier covers 340 app routes and 30 critical aliases, and `/design-lab/occupancy` emits statically with only documented repository warnings.
- **Parity:** The slice is synthetic and additive. No production class, child, branch, compliance, attendance, daily report, accounting, payment, query, mutation, server action, Prisma model, database row, permission, route, export, legacy PHP alias, parent/native payload, or restored capability changed.
- **Evidence boundary:** Approved first-market session/capacity/staffing/funding/billing policy, authoritative booking sources, historical schedule availability, production persistence and transactions, hold concurrency, waitlist/admissions rules, cancellation/notice policy, source adapters, authorization integration, representative scale, operator validation, assistive technology, native/parent continuity, and selected-system polish remain release gates.
- **Next action:** Continue the next reversible Wave 3/5 domain foundation while awaiting explicit Daylight, Signal, or Carebook selection; production visual migration and Phase 6/7 credit remain gated on that choice.
- **Progress earned:** 0 percentage points. This closes occupancy/future-place behavioral uncertainty but does not satisfy selected-direction production implementation required for Phase 6 or rollout required for Phase 7; total remains 55%.

### 2026-07-11 - Wave 3 accountable staffing and rota foundation

- **Question:** Can Kiddz prove room staffing readiness and resolve an absence or due break without relabeling a static teacher/class link, calendar event, profile document, or scanner log as a complete rota?
- **Evidence:** Current Teacher, TeacherDocument/Attachment/Experience, EmployeeEvent, and TeacherAttendance schema/actions; staff directory/detail, monthly calendar, attendance import, and attendance-log surfaces; J05 workflow blueprint; direct-competitor staffing synthesis; deterministic contract verifier; Agent Browser role/state/viewport matrix and live transitions.
- **Decision:** Keep availability, shift, room assignment, presence, effective qualification, reported/confirmed absence, break, cover selection, cover assignment, and accepted event history separate. Derive room contribution only from explicit current facts and reject cover that resolves the target by creating a source-room gap.
- **Artifacts:** `src/lib/redesign-staffing-rota-contracts.ts`, `src/scripts/verify-redesign-staffing-rota-contracts.ts`, `/design-lab/staffing`, `docs/redesign/staffing-rota-contract.md`, and linked master-plan, architecture, and decision-log updates.
- **Verification:** Deterministic checks prove nine lifecycle states, complete non-regressing sources, overlap resolution, absence confirmation, exact room contribution, expired-qualification rejection, target/source candidate consequences, temporary cover, covered breaks, idempotency/revision handling, role privacy, stale-source recovery, and capability denial. Agent Browser covers nine states across manager `1440 x 900`, rota coordinator `768 x 1024`, and practitioner `390 x 844` for 27 combinations; every run has its expected heading, one H1, zero overflow, unnamed/undersized controls, axe violations, or incomplete findings. Manager private evidence remains visible, coordinator private evidence is removed while consequence remains, and practitioner runs expose no colleague identity or private reason. The nine-step manager lifecycle returns focus, announces each result, and produces no errors; desktop/mobile visual scrolling is coherent and browser logs are empty. Focused ESLint, full TypeScript, all 16 redesign suites, all 14 staff/teacher/employee/attendance/absence parity suites, diff hygiene, and the production build pass; the route verifier covers 341 app routes and 30 critical aliases, and `/design-lab/staffing` emits statically with only the documented middleware, CSS `@page`, and dynamic-auth prerender warnings. Six stale parity assertions were corrected to validate the current consolidated restored status plus exact verification evidence instead of obsolete status fragments; no parity-matrix or production behavior changed.
- **Parity:** The slice is synthetic and additive. No production staff, event, attendance, calendar, class, branch, ratio, payroll, query, mutation, server action, Prisma model, database row, permission, route, export, PHP alias, parent/native payload, or restored capability changed.
- **Evidence boundary:** Approved qualification/break/ratio/substitute policy, authoritative sources, branchless/cross-site rules, production persistence and transactions, attendance deduplication, payroll effects, concurrency, emergency overrides, source adapters, authorization integration, representative scale, operator validation, assistive technology, real devices, and selected-system polish remain release gates.
- **Next action:** Continue the next reversible canonical workflow or hardening foundation while awaiting explicit Daylight, Signal, or Carebook selection; production visual migration and Phase 6/7 credit remain gated on that choice.
- **Progress earned:** 0 percentage points. This closes J05/staffing behavioral uncertainty but does not satisfy selected-direction production implementation required for Phase 6 or rollout required for Phase 7; total remains 55%.

### 2026-07-11 - J03 accountable daily-care foundation

- **Question:** Can Kiddz preserve fast room reporting without inventing care values, counting drafts as done, creating a second absence truth, partially deleting nested evidence, or claiming parent communication before delivery?
- **Evidence:** DailyReport, fever/milk/attachment, AbsenceReport, direct-approval, and legacy-provenance schema/actions; batch/full forms; list/draft/edit/print/export routes; parent API and `/ws/daily.php`/`/ws/newdaily.php` adapters; J03 audit/blueprint; reliability and parent/native contracts; parity rows.
- **Decision:** Begin factual fields unset; inherit shared values only after exact present-child review; preserve field-level exceptions; autosave immutable draft revisions; resolve conflicts without unrelated overwrite; submit selected complete reports atomically against fresh attendance; create separate retryable delivery jobs; expose parents only to delivered relation-scoped revisions; append reasoned corrections.
- **Artifacts:** `src/lib/redesign-daily-care-contracts.ts`, `src/scripts/verify-redesign-daily-care-contracts.ts`, `/design-lab/daily-care`, `docs/redesign/daily-care-contract.md`, and linked blueprint, architecture, master-plan, and decision-log updates.
- **Verification:** Deterministic checks prove all 12 states, no initial facts, complete/non-regressing sources, absent/unknown exclusion, explicit shared inheritance, retained exception provenance, atomic drafts, field-governed offline queueing, cross-device merge, stale-source blocking, all-or-nothing submission, separate retryable delivery, role-safe parent projection, append-only correction, idempotency, and capability denial. Agent Browser covers all 12 states across manager `1440 x 900`, practitioner `768 x 1024`, and linked parent `390 x 844` for 36 combinations; every run has one H1, expected role heading, zero overflow, unnamed/undersized controls, axe violations, or incomplete findings. Empty capture exposes no fake values. Parent runs expose only Amelie and delivered parent-visible revisions, with no peer identity, staff source/draft/conflict/history/provider/recipient data; practitioner runs expose no parent account identity. The 12-action manager lifecycle returns focus, announces every result, and produces no errors; desktop/mobile visual scrolling is coherent and browser logs are empty. Focused ESLint, full TypeScript, all 17 redesign suites, six daily-report/parent parity suites, diff hygiene, and the production build pass; the parent verifier uses a throwaway process-only test secret, the route verifier covers 342 app routes and 30 critical aliases, and `/design-lab/daily-care` emits statically with only the documented middleware, CSS `@page`, and dynamic-auth prerender warnings.
- **Parity:** The slice is synthetic and additive. No production daily report, nested fever/milk/attachment evidence, absence, attendance, roster/list/draft, approval, print/export, action, Prisma model, database row, permission, route, PHP alias, parent/native payload, or restored capability changed.
- **Evidence boundary:** Approved care/completeness/health/offline/consent policy, direct-approval interpretation, production persistence and atomic nested transactions, outbox/provider integration, delete/archive/retention, source adapters, authorization integration, representative scale, shared-device interruption, operator/parent validation, assistive technology, real devices/native clients, and selected-system polish remain release gates.
- **Next action:** Continue the next reversible canonical workflow or hardening foundation while awaiting explicit Daylight, Signal, or Carebook selection; production visual migration and Phase 6/7 credit remain gated on that choice.
- **Progress earned:** 0 percentage points. This closes J03/daily-care behavioral uncertainty but does not satisfy selected-direction production implementation required for Phase 6 or rollout required for Phase 7; total remains 55%.

### 2026-07-11 - Wave 4 accountable family-communication foundation

- **Question:** Can direct, class, and broadcast communication remain fast without treating a message row as delivery proof, a read flag as a reply, a shared campaign thread as a safe family conversation, or deletion as acceptable audit history?
- **Evidence:** Message/MessageThread, Notification/Receipt, PushToken, parent relationship, call, and alarm/event schema; staff direct/class/bulk/reply/read/delete/resend actions; external channel audit and legacy-nature side effects; parent/native list/thread/send adapters and parser contracts; Calls placement; reliability, Action Center, permission, route, and parity evidence.
- **Decision:** Keep versioned content, source-linked audience, explicit exclusions, approval, publication acceptance, per-family conversation, recipient/channel attempt, read, reply, follow-up, archive, and correction separate. Campaign grouping never grants conversation access; parent projection requires its own active relationship and delivered message revision.
- **Artifacts:** `src/lib/redesign-communication-contracts.ts`, `src/scripts/verify-redesign-communication-contracts.ts`, `/design-lab/communication`, `docs/redesign/communication-contract.md`, and linked architecture, master-plan, blueprint, and decision-log updates.
- **Verification:** Deterministic checks prove all 12 states, no premature recipient/publication/delivery facts, complete/non-regressing sources, versioned content, inactive account/relation exclusions, consented channel resolution, approval binding, stale-audience rejection/refresh, atomic publication/conversation/outbox creation, one isolated conversation per family, exact idempotency, partial delivery, failed-only retry, read/reply separation, relation-scoped reply, participant archive without deletion, frozen-audience correction, role privacy, pending-correction withholding, and capability denial. Agent Browser covers all 12 states across manager `1440 x 900`, practitioner `768 x 1024`, and linked parent `390 x 844` for 36 combinations; every run has one H1, expected role heading, zero overflow, unnamed/undersized controls, axe violations, or incomplete findings. Parent runs expose no peer identity, internal source/account/provider data, staff draft, or pending correction; practitioner runs expose no other family or provider detail. The nine-step manager lifecycle and linked-parent reply return focus, announce each result, preserve isolated conversation counts, and produce no errors. Visual review caught and corrected min-content mobile heading/top-control collapse; the full matrix rerun and desktop/mobile captures pass with empty warning/error logs. Focused ESLint, full TypeScript, all 18 redesign suites, 16 message/parent/native/legacy-side-effect/Calls parity contracts, diff hygiene, and the production build pass; database-backed parity checks use a throwaway process-only test secret, the route verifier covers 343 app routes and 30 critical aliases, and `/design-lab/communication` emits statically with only the documented middleware, CSS `@page`, dynamic-auth prerender, and PostgreSQL SSL-mode migration warnings.
- **Parity:** The slice is synthetic and additive. No production message, thread, notification, alarm/event/holiday side effect, provider attempt, read flag, query, mutation, Message/MessageThread schema, database row, route, permission, PHP alias, parent/native payload, CallLog record, or restored capability changed.
- **Evidence boundary:** Immediate shared-thread cross-family projection remediation; approved communication category, approval, consent, suppression, quiet-hour, emergency, reply, escalation, archive/void/retention/redaction/erasure/legal-hold policy; production persistence, transactions/outbox, provider credentials/webhooks, source adapters, authorization, backfill, scale, operator/parent validation, assistive technology, real devices, native clients, and selected-system polish remain release gates.
...
### 2026-07-11 - Wave 6 accountable academic-rollover foundation

- **Question:** Can Kiddz preserve the restored legacy year transition while preventing an uploaded file, one transaction, or one actor from being mistaken for a recoverable and complete rollover?
- **Evidence:** Current new-year and school-year actions; SQL dump engine and object-storage path; `SchoolYear`, `ChildHistory`, and `LegacyYearDatabase` schema; `/settings/new-year`, `/newyear.php`, and `/exportdb.php`; legacy `ArchiveAndCreate` source; restoration matrix and verifier; inspection-package backup separation; authorization, reliability, data-delivery, and Wave 6 architecture evidence.
- **Decision:** Bind a versioned carry-forward plan to complete source revisions and read-only preflight; separate impact confirmation, approved immutable backup, isolated restore proof, distinct operator/data-custodian approvals, fresh-source organization lock, atomic execution receipt, post-cutover validation, verified rollback, and append-only audit. A fallback SQL export remains compatibility evidence but cannot authorize production rollover without approved equivalent restore proof.
- **Artifacts:** `src/lib/redesign-academic-rollover-contracts.ts`, `src/scripts/verify-redesign-academic-rollover-contracts.ts`, `/design-lab/academic-rollover`, `docs/redesign/academic-rollover-contract.md`, and linked architecture, blueprint, master-plan, decision-log, and progress updates.
- **Verification:** Deterministic checks prove all 12 states, complete/non-regressing sources, invalid-date and blocker handling, zero preflight mutation, exact impact, immutable/checksummed metadata, fallback-engine and incomplete-restore rejection, distinct revision-bound approvals, exact idempotency, stale-source/lock execution rejection, accepted counts, failed validation, rollback, completion, privacy, and capability denial. Agent Browser covers all 12 states across administrator `1440 x 900`, coordinator `768 x 1024`, and auditor `390 x 844` for 36 synchronized combinations; every run has one H1, zero overflow, unnamed/undersized user controls, axe violations, incomplete findings, or role-projection leaks. The nine-step full lifecycle, blocked-plan correction, and failed-validation rollback return focus, announce results, preserve approval separation, and produce no UI error; the auditor execution control is disabled and identity-safe. Desktop and mobile visual captures are coherent. Focused ESLint, full TypeScript, all 19 redesign suites, legacy new-year/archive, settings, and admin-export parity, diff hygiene, and the production build pass; route verification covers 344 app routes and 30 critical aliases, and the lab emits statically with only the documented middleware, CSS `@page`, and dynamic-auth prerender warnings.
- **Parity:** The slice is synthetic and additive. No production school year, child, teacher, class, branch, history, registry, legacy year database, SQL dump, backup object, query, mutation, action, Prisma model, database row, permission, route, PHP alias, export, parent/native payload, or restored capability changed.
...
### 2026-07-13 - Phase 3 finalist type and color readiness

- **Question:** Can both finalists become production-ready across display and product typography, licensing and delivery, Arabic/RTL, 200% text, mixed-script numerals, and semantic color without quietly locking the brand?
- **Evidence:** Existing Arabic product fields and audit requirements; Kinetic Kindness and Living Record proof surfaces; official Google Fonts metadata and SIL Open Font License files for Fredoka, Newsreader, Inter, Noto Sans Arabic, Readex Pro, and Noto Naskh Arabic; official Next.js font-delivery documentation; and the existing two-finalist route and workflow verifier.
- **Decision:** Fredoka, Newsreader, and Inter are legally feasible OFL families that can be self-hosted through `next/font`, but none covers Arabic. Keep the Arabic family open: use Noto Sans Arabic only as the neutral product-baseline candidate, with Readex Pro as the Kinetic expressive candidate and Noto Naskh Arabic as the Living editorial candidate. The shortlist and ranking remain unchanged, and no production font or brand direction is selected.
- **Artifacts:** `docs/redesign/finalist-type-color-readiness.md`; a shared role, scale, language, numeral, licensing, and semantic-color appendix on `/design-lab/brand-directions/finalists`; finalist type metadata and query-state support; and expanded finalist proof verification.
- **Verification:** Agent Browser covered both finalists at `1440 x 900` and `390 x 844`, at 100% and 200% proof scales, for eight controlled states. Every state rendered the correct finalist and type roles, Arabic `lang`/RTL treatment and isolated mixed numerals, four labelled semantic states, one H1, zero page or proof-grid overflow, zero undersized visible targets, zero axe violations, and zero incomplete findings. Both finalist workflows were then regressed through cover disclosure, empty-note validation, approval, and parent preview. Focused ESLint, full TypeScript, the finalist and six-system verifiers, 347-route/30-alias compatibility, diff hygiene, and the production build pass.
- **Parity:** The slice changes only design-lab data, route presentation, documentation, and deterministic verification. No production behavior, component, route contract, query, mutation, database row, permission, legacy alias, native/parent payload, or restored capability changed.
- **Evidence boundary:** Browser proof scaling is not operating-system or browser zoom. Real Arabic font rendering and translation review, assistive-technology testing, PDF/email/native projection, font-payload measurement, trademark review, operator testing, legal approval, and the explicit brand lock remain open gates.
- **Next action:** Explicitly select Kinetic Kindness or Living Record, then codify the chosen brand/design constitution with a tested Arabic family before creating production tokens, component APIs, or page migrations.
- **Progress earned:** 0 percentage points. This closes finalist type, licensing, script, scale, numeral, and semantic-color uncertainty, but the irreversible brand selection remains open; total remains 55%.

### 2026-07-13 - Phase 3 equal-footing finalist identity worlds

- **Question:** Can Kinetic Kindness and Living Record become complete, ownable brand worlds across strategy, mark, memory asset, voice, image language, motion, and staff/family/brand applications without selecting production styling or changing the shared care truth?
- **Evidence:** The approved Kiddz Online Remotion render and O-ring seed; identical finalist product and type/color proofs; the user's white-dominant, colorful, purposeful-motion brief; Pinterest and Mobbin synthesis; Ehrenberg-Bass distinctive-asset guidance; Duolingo identity, writing, and illustration governance; ITAL/C's Headspace brand-evolution case; Apple motion guidance; and Vercel brand-asset and semantic-color guidance.
- **Decision:** Keep the approved animation as the common seed. Advance Kinetic's living O-ring/purposeful dot, consequence-aware warmth, care-constellation image grammar, and leave/carry/return motion as the research lead. Preserve Living Record's dated care mark, authored margin, observed-care image grammar, and reveal/date/preserve motion as the stronger evidence-led alternative. Do not select production identity, create a hybrid, or mistake concept assets for recognition, trademark, or commissioned-production evidence.
- **Artifacts:** `docs/redesign/finalist-identity-worlds.md`; `/design-lab/brand-directions/identity-worlds`; typed two-world strategy, palette, voice, image, motion, application, and kill-condition data; public approved logo render, poster, and English caption track; responsive styling; and `src/scripts/verify-redesign-identity-worlds.ts`.
- **Verification:** Agent Browser verified both direct finalist URLs at `1440 x 900` and `390 x 844`. All four states rendered one H1, one active finalist, six color roles, four consequence-aware voice examples, three applications, loaded video metadata, one caption track, zero page/inner overflow, zero undersized visible controls, zero axe violations, and zero incomplete findings. Finalist switching preserves unrelated query state; both motion signatures replay, announce, and settle; the approved logo seed plays from the start. The identity-world, finalist, and six-system verifiers, focused ESLint, full TypeScript, 348-route/30-alias compatibility, diff hygiene, and the production build pass. Only the documented middleware, CSS `@page`, and authenticated dynamic-render warnings remain.
- **Parity:** The slice adds an isolated design-lab route, preserved approved media, research documentation, and deterministic verification. No production component, route contract, query, mutation, database row, permission, legacy alias, native/parent payload, or restored capability changed.
