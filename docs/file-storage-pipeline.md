...
Branch compliance upload screens use this route and then persist the returned `publicUrl` on `BranchDocument.fileUrl`. Accounting payment dialogs use the same route with `payment-receipt` scope and persist the returned `publicUrl` on `Payment.receiptFileUrl`. Absence report forms use `absence-report` scope and persist the returned URLs on `AbsenceAttachment.fileUrl`. Daily report forms use `daily-report` scope and persist the returned URLs on `DailyReportAttachment.fileUrl`. Employee document rows use the role-specific document scopes before persisting teacher/nurse/doctor document URLs.
