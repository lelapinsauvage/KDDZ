...
The J03 daily-care boundary is now executable in `src/lib/redesign-daily-care-contracts.ts` and `/design-lab/daily-care`, with evidence in `daily-care-contract.md`. It proves unset factual capture, explicit shared inheritance, child exceptions, canonical attendance references, versioned drafts, field-governed offline queueing, cross-device conflict resolution, fresh-source atomic submission, retryable parent delivery, role-safe publication, and append-only correction. It remains synthetic and additive; approved care policy, production persistence/transactions/outbox, source adapters, authorization, operator/parent validation, native delivery, and parity migration are still open.
...
The Wave 4 communication boundary is now executable in `src/lib/redesign-communication-contracts.ts` and `/design-lab/communication`, with evidence in `communication-contract.md`. It proves source-linked drafts, consented channel eligibility, explicit exclusions, approval-bound recipient snapshots, isolated family conversations, atomic publication/outbox creation, failed-only delivery retry, read/reply separation, participant archive, role-safe projections, and append-only correction. It remains synthetic and additive; immediate shared-thread privacy remediation, approved communication/consent/retention policy, production persistence/transactions/outbox, source adapters, authorization, provider rollout, operator/parent validation, native acceptance, and parity migration remain open.
