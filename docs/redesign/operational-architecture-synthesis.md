...
| OccupancyPlan, Booking, Hold, and CapacityBlock | Trustworthy future places | session, capacity sources, confirmed booking, hold expiry, blocks, sellable places, source revision |
...
The territory-neutral room-care session is now executable in `src/lib/redesign-daily-care-contracts.ts` and `/design-lab/daily-care`, with evidence in `daily-care-contract.md`. It keeps attendance, observations, shared provenance, exceptions, draft revisions, sync conflicts, submission, parent delivery, and corrections distinct while preserving the parent/native compatibility boundary. It remains synthetic and additive; approved care policy, production persistence/transactions/outbox, source adapters, authorization, operator/parent validation, native delivery, and parity migration are still open.

The territory-neutral communication lifecycle is now executable in `src/lib/redesign-communication-contracts.ts` and `/design-lab/communication`, with evidence in `communication-contract.md`. It keeps content drafts, audience snapshots, approval, campaign grouping, family conversations, channel attempts, read receipts, replies, follow-ups, archives, and corrections distinct while preserving legacy message natures and parent/native compatibility as migration gates. It remains synthetic and additive; immediate shared-thread privacy remediation, policy, production persistence/transactions/outbox, source adapters, authorization, provider rollout, operator/parent validation, native acceptance, and parity migration are still open.
...
The territory-neutral occupancy plan is now executable in `src/lib/redesign-occupancy-planning-contracts.ts` and `/design-lab/occupancy`, with evidence in `occupancy-planning-contract.md`. It keeps current roster, confirmed bookings, live attendance, physical/policy/staffing capacity, named blocks, expiring holds, future sellable places, and booked/attended/claimed/invoiced minutes distinct while binding every decision to source revisions. It remains synthetic and additive; production session/booking sources, approved capacity and funding policy, persistence, transactions, authorization, operator validation, and parity migration are still open.
