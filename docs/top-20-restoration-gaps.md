...
8. **Actual notification sending**
   - Modern app now has an in-app template dispatch path with variable rendering, sent-log visibility, resend, per-template test send to the current user, provider-neutral external email delivery for notification tests/resends and legacy bulk email, and the same provider bridge wired into legacy forgot/reset, signup/activation, profile confirmation, disabled-contact, and admin-created-user emails. Configure it with `EMAIL_DELIVERY_PROVIDER=webhook|resend`, `EMAIL_DELIVERY_WEBHOOK_URL` or `RESEND_API_KEY`, and `EMAIL_FROM`; unconfigured delivery is reported as skipped instead of silently succeeding.
