...
8. **Actual notification sending**
   ...
   - Payment alarm generation now restores the `newpayment`/`PaymentReminder` path from `Data::AlarmsPaidPayments()` as a cron-safe/manual path that creates idempotent child-specific `Alarm` rows visible to parent payment feeds and exposes `/api/cron/payment-alarms`.
   - Remaining work is idempotent cron/job generation for the other approved legacy families, payment before/after due reminders, hosted schedule configuration after production crontab confirmation, and external push/email/SMS/WhatsApp providers after credential recovery or rotation.
