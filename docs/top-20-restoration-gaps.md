...
20. **Role and permission parity**
   ...
   - `/profile`, `/profile.php`, and `/users/profile.php` now restore the signed-in PHP-login profile flow with current-password validation, editable name/email/password, migrated profile fields, access logs, integration status, and `update_emailPw` confirmation-token handling.
   - `/login` now appends migrated-compatible legacy login timestamp rows for linked PHP-login users when legacy profile timestamps are enabled, restoring the audit source used by access logs and login reports.
