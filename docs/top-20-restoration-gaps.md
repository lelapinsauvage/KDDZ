...
8. **Actual notification sending**
   - Modern app now has an in-app template dispatch path with variable rendering, sent-log visibility, resend, per-template test send to the current user, and provider-neutral external email delivery for notification tests/resends and legacy bulk email. Configure it with `EMAIL_DELIVERY_PROVIDER=webhook|resend`, `EMAIL_DELIVERY_WEBHOOK_URL` or `RESEND_API_KEY`, and `EMAIL_FROM`; unconfigured delivery is reported as skipped instead of silently succeeding.
