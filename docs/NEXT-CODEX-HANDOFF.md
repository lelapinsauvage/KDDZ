...
- `6a3c4e4 chore: enforce zero blockers in final evidence packages`
- `41f87d2 chore: require zero-blocker final evidence artifacts`
- `351be87 chore: verify release-bound preflight env commands`
- `28d2978 chore: bind closeout env templates to release metadata`
- `c29704e chore: include work orders in closeout env templates`
- `7faacd7 chore: validate production evidence json pointers`
- `d4e238b chore: validate local migration evidence artifacts`
...
Commit `4440183` added `--gate=<gate>` filtering to `report-production-partials.ts`. Use it to generate the four non-secret row-coverage artifacts required by the remaining external gates: `--gate=PROD-CRON`, `--gate=PROD-PROVIDERS`, `--gate=PROD-NATIVE`, and `--gate=PROD-NATURE`. Commit `b593e4c` added matching `summary.gateFilter` metadata to focused evidence checklists and taught `verify-production-artifact-consistency-contract.ts` to verify a focused partial report against the focused checklist for the same gate. The consistency verifier also requires both artifacts to name the same gate-map and production-gates source paths. Commit `63db45e` made the cutover runbook require focused partial/checklist pairs and consistency verification for all four remaining focused gates. Commit `df68947` added `report-production-focused-artifacts.ts --out-dir=<dir>` to generate the whole focused artifact bundle with one frozen timestamp and a digest manifest. Commit `317b06e` added `verify-production-focused-artifacts-manifest.ts --manifest=<path>` so archived focused bundles can be hash-checked and consistency-checked later; saved-manifest verification now also requires every nested focused partial/checklist artifact to match the manifest timestamp, source matrix/gate-map/production-gates paths, and gate filter. `verify-production-focused-artifacts-contract.ts` and `verify-production-focused-artifacts-manifest-contract.ts` prove both generation and saved-manifest verification. The full, unfiltered partial report is still the artifact used by final closeout.

### Zero-Partial Closeout Hardening

Commits `93d580f`, `e378e55`, `663bd0e`, and `cacf7ef chore: fail zero-partial closeout early` hardened final production closure. Artifact consistency no longer hardcodes today's 17 partial rows, and the closeout, closeout summary, and evidence package contracts now build controlled zero-partial fixtures to prove the final `--require-zero-partials` package can pass when `docs/page-parity-matrix.json` truly has no partial rows. The closeout command now fails immediately after generating and consistency-checking the partial/checklist artifacts when `--require-zero-partials` sees unresolved partial rows, before validating the final acceptance record, so the failure reports the blocker count directly. The production gate suite also derives its tracker assertion from `report-production-partials.ts --json` instead of a fixed partial-row count, so real production evidence closure can reduce the remaining rows without editing the suite. The normal closeout path still reads `docs/page-parity-matrix.json`, `docs/partial-production-gate-map.md`, and `docs/legacy-production-acceptance-gates.md` by default; `--parity-matrix=<path>`, `--partial-gate-map=<path>`, and `--production-gates=<path>` are documented for controlled archived evidence reproduction and contract tests only.
...
Commits `d9690f9`, `1b5f27c`, `da487a0`, `fc63539`, `378a1ae`, `9b2db15`, `4a1745b`, `1c37842`, `b40bfe3`, `3ccf0cb chore: expose production gate closure links`, and `41f87d2 chore: require zero-blocker final evidence artifacts` made the final evidence closure auditable before cutover. `report-production-gate-status.ts --require-ready` now joins readiness, partial rows, and checklist status into a redacted closure board that fails until every production gate is ready. Add `--require-no-blockers` for final closure so the same board also fails while any partial parity rows remain. The gate-status JSON includes `sourceAlignment.status=verified` after proving the readiness audit, partial report, and evidence checklist share the frozen timestamp and matching per-gate blocker rows. Use `report-production-gate-status.ts --blocking-only` to focus that board on the four gates still blocking page-parity rows; the current local summary reports 17 unique blocking partial rows, 27 blocking gate links, `closeoutMode: "external-production-evidence"`, and `canCloseLocally: false` until real production evidence closes the gates. Gate status entries now include redacted `nextActions` with missing evidence pointer names, focused partial/checklist archive commands, and the mapped partial rows each gate can unblock. `render-production-acceptance-evidence-record.ts` fills `docs/production-acceptance-evidence-template.md`, computes readiness, partial, checklist, and preflight manifest SHA-256 values, records that the closeout summary hash is verified in the evidence package manifest, binds the release branch and commit, and immediately verifies the filled record with `--require-zero-artifacts`. Closeout, closeout-summary verification, and evidence-package verification now all carry the same archived `--preflight-manifest=<path>` and digest so the final release record, closeout summary, and package manifest bind the exact preflight bundle. The acceptance verifier can require `--preflight-manifest=<path>`, `--preflight-digest=<sha256>`, and `--require-zero-artifacts` so the final evidence record names the exact archived preflight bundle manifest and rejects partial/checklist/preflight artifacts that still carry blocking rows. `verify-production-gate-status-contract.ts`, `verify-production-acceptance-evidence-record-contract.ts`, and `verify-production-acceptance-evidence-renderer-contract.ts` are part of `pnpm run verify:production-gates`.
...
Commits `388567c`, `8557360`, `ee2b8ce`, and `06aa0df chore: preserve preflight closeout summary` added and hardened `report-production-preflight-artifacts.ts --out-dir=<dir>` plus `verify-production-preflight-artifacts-manifest.ts --manifest=<path>`. Use them to create and recheck the archived non-secret preflight bundle containing the full partial report, full evidence checklist, blocker-only gate status report, focused artifact manifest, closeout plan, SHA-256 digests, and top-level `blockingGateSummary` before collecting private production evidence. The saved preflight and focused-manifest verifiers honor the source matrix/gate-map/production-gates paths recorded in the archived manifests, and the preflight verifier now requires the blocker-status report plus nested focused manifest and closeout plan to match those recorded source paths, requires the blocker-status report and closeout plan to retain verified source alignment, requires all bundled JSON artifacts to share the preflight `generatedAt`, and rejects drift between `blockingGateSummary` and the nested blocker-only gate-status report, including total and per-gate `blockingGateLinks`, `closeoutMode`, and `canCloseLocally`, so controlled reproduction bundles do not have to point at the live default docs.

Commits `82690a9 chore: expose preflight summary in evidence package`, `906643e chore: verify package gate blocker counts`, and `6a3c4e4 chore: enforce zero blockers in final evidence packages` now carry that archived preflight `blockingGateSummary` into the final evidence package manifest too. The package manifest also carries the nested preflight closeout-plan artifact hash. Saved package verification rejects drift if the packaged summary stops matching the archived preflight manifest, including `blockingGateLinks`, `closeoutMode`, and `canCloseLocally`, so the release package cannot silently claim local closeout while the preflight bundle still says external production evidence is required. It also rejects drift in the per-gate blocker counts under `gatesToClose` and the nested closeout-plan artifact digest. In final `--require-zero-partials` mode, evidence package verification now additionally requires the closeout partial summary, checklist blocking count, preflight blocking rows, preflight blocking gate links, `ready-for-final-closeout`, and `canCloseLocally: true` to agree before a saved final package manifest is accepted.

Commits `d4e238b`, `7faacd7`, `c29704e`, `28d2978`, and `351be87` hardened the evidence-pointer and release-binding layer. `audit-production-readiness.ts` now shape-checks local JSON evidence pointers for migration/file artifacts and production evidence artifacts without printing paths or contents; malformed local JSON evidence keeps the gate in `needs-evidence`, while external archive/ticket IDs still count as configured pointers. The closeout plan now emits per-gate readiness env-template commands with `--include-work-orders`, `--generated-at=<iso>`, `--release-branch=<branch>`, `--release-commit=<sha>`, and `--acceptance-date=<YYYY-MM-DD>`. The preflight manifest verifier rejects release metadata drift in those archived closeout-plan env-template commands.
