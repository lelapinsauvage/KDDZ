...
export async function POST(request: NextRequest) {
  const [from, brid] = await Promise.all([
    readField(request, "from"),
    readField(request, "brid"),
  ]);
  const branchId = brid ? await resolveLegacyBranchId(brid) : null;

  if (!branchId) {
    return htmlResponse("Branch not found", 404);
  }
