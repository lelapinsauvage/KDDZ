...
function formatGenerationStatus(
  ...
  if (family === "vaccination") {
    ...
    const notificationsCreated = metric(data, "notificationsCreated");
    const skippedExisting = metric(data, "skippedExisting");
    return `Matched ${remindersMatched}; created ${alarmsCreated} alarm${alarmsCreated === 1 ? "" : "s"}, ${receiptsCreated} receipt${receiptsCreated === 1 ? "" : "s"}, and ${notificationsCreated} notification${notificationsCreated === 1 ? "" : "s"}; skipped ${skippedExisting} existing.`;
  }
