import { notFound, redirect } from "next/navigation";
import { resolveLegacyChildId } from "@/lib/legacy-child";
import { resolveLegacyMedicalFormId } from "@/lib/legacy-medical-form";

interface PageProps {
  searchParams: Promise<{ fid?: string; id?: string; tid?: string }>;
}

export default async function LegacyMedicalVisitFormRedirect({
  searchParams,
}: PageProps) {
  const { fid, id } = await searchParams;

  if (fid?.trim()) {
    const formId = await resolveLegacyMedicalFormId("VISITS", fid);
    if (!formId) notFound();

    redirect(`/medical/visits/${encodeURIComponent(formId)}`);
  }

  if (id?.trim()) {
    const childId = await resolveLegacyChildId(id);
    if (!childId) notFound();

    redirect(`/medical/visits/new?childId=${encodeURIComponent(childId)}`);
  }

  redirect("/medical/visits");
}
