...
function addSummary(
  total: AssessmentGenerationSummary,
  next: AssessmentGenerationSummary,
) {
...
async function runAssessmentCron(request: NextRequest) {
  ...
  for (const target of resolved.targets) {
    const summary = await generateAssessmentAlarmsForOrganization(target);
...
export async function GET(request: NextRequest) {
  return runAssessmentCron(request);
}
