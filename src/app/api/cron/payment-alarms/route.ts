...
function addSummary(
  total: PaymentGenerationSummary,
  next: PaymentGenerationSummary,
) {
...
async function runPaymentCron(request: NextRequest) {
  ...
  for (const target of resolved.targets) {
    const summary = await generatePaymentAlarmsForOrganization(target);
