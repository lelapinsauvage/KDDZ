...
function addSummary(
  total: InsuranceGenerationSummary,
  next: InsuranceGenerationSummary,
) {
  total.branchesScanned += next.branchesScanned;
  total.formsScanned += next.formsScanned;
  total.formsMatched += next.formsMatched;
...
async function runInsuranceCron(request: NextRequest) {
  ...
  for (const target of resolved.targets) {
    const summary = await generateInsuranceAlarmsForOrganization(target);
...
export async function GET(request: NextRequest) {
  return runInsuranceCron(request);
}
