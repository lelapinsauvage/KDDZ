...
function addSummary(
  total: MedicineGenerationSummary,
  next: MedicineGenerationSummary,
) {
...
async function runMedicineCron(request: NextRequest) {
  ...
  for (const target of resolved.targets) {
    const summary = await generateMedicineAlarmsForOrganization(target);
...
export async function GET(request: NextRequest) {
  return runMedicineCron(request);
}
