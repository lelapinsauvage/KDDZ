...
function addSummary(
  total: MedicalGenerationSummary,
  next: MedicalGenerationSummary,
) {
  total.branchesScanned += next.branchesScanned;
  total.childrenScanned += next.childrenScanned;
  total.reportsMatched += next.reportsMatched;
...
async function runMedicalCron(request: NextRequest) {
  ...
  for (const target of resolved.targets) {
    const summary = await generateMedicalAlarmsForOrganization(target);
