...
function addSummary(
  total: ContractGenerationSummary,
  next: ContractGenerationSummary,
) {
  ...
  total.skippedMissingExpiry += next.skippedMissingExpiry;
...
async function runContractCron(request: NextRequest) {
  ...
  for (const target of resolved.targets) {
    const summary = await generateContractAlarmsForOrganization(target);
...
export async function GET(request: NextRequest) {
  return runContractCron(request);
}
