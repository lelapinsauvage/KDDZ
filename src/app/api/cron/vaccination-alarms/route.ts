...
function addSummary(
  total: VaccinationGenerationSummary,
  next: VaccinationGenerationSummary,
) {
...
async function runVaccinationCron(request: NextRequest) {
  ...
  for (const target of resolved.targets) {
    const summary = await generateVaccinationAlarmsForOrganization(target);
...
export async function GET(request: NextRequest) {
  return runVaccinationCron(request);
}
