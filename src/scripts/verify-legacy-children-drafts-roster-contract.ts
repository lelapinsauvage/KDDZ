...
const text = {
  ...
  legacyJs: readFileSync(
    `${legacyRoot}/Front/templates/admin/js/children_drafts.js`,
    "utf8",
  ),
  bridge: readFileSync("src/app/(app)/children_drafts.php/page.tsx", "utf8"),
  page: readFileSync("src/app/(app)/children/drafts/page.tsx", "utf8"),
  client: readFileSync(
...
assert.match(text.legacyPhp, /Check::protectPageOrFunction\('children_drafts\.php'\)/);
...
assert.match(text.legacyPhp, /Children Drafts/);
assert.match(text.legacyPhp, /Children Drafts Listing/);
...
assert.match(
  text.legacyPhp,
  /Image[\s\S]*F Name[\s\S]*L Name[\s\S]*DOB[\s\S]*Branch[\s\S]*Class[\s\S]*Nationality[\s\S]*Gender[\s\S]*Date[\s\S]*Action/,
);
...
assert.match(text.legacyJs, /getChildrenDraftHashed/);
...
assert.match(text.legacyJs, /d\.columns\[9\]\.Min_Range/);
assert.match(text.legacyJs, /d\.columns\[9\]\.Max_Range/);

assert.match(text.bridge, /redirect\("\/children\/drafts"\)/);
assert.match(text.guards, /legacyPage: "children_drafts\.php"/);
assert.match(text.guards, /"\/children_drafts\.php"/);
assert.match(text.guards, /"\/children\/drafts"/);

assert.match(text.page, /params\.pageSize === "all" \? "all" : Number\(params\.pageSize\) \|\| 10/);
assert.match(text.page, /getDrafts\(\{/);
assert.match(text.page, /childNumber: params\.childNumber/);
assert.match(text.page, /nationality: params\.nationality/);
...
assert.match(text.client, /title="Children Drafts"/);
assert.match(text.client, /Children Drafts Listing/);
assert.match(text.client, /New Child/);
...
assert.match(text.client, /const showingFrom =/);
assert.match(text.client, /const showingTo =/);
...
assert.match(text.client, /Select all drafts in page/);
assert.match(text.client, /bulkUpdateChildrenBranchClass/);
assert.match(text.client, /deleteChild/);
assert.match(text.client, /filename="children_drafts"/);
assert.match(text.client, /sheetName="Children Drafts"/);
...
assert.match(text.columns, /variant === "drafts"/);
...
assert.match(text.columns, /SortableHeader column=\{column\}>Nationality/);
...
assert.match(text.actions, /export async function getDrafts/);
...
const row = matrix.find(
  (entry) => entry.legacyPhp === "Front/templates/admin/children_drafts.php",
);
assert.ok(row, "page parity row for children_drafts.php");
assert.equal(
  row.status,
  "restored - legacy draft roster, bulk branch/class modal, filters, exports, ACL, and bridge restored",
);
...
assert.match(row.verification, /verify-legacy-children-drafts-roster-contract\.ts/);
assert.match(
  text.matrixMd,
  /children_drafts\.php \| Front\/templates\/admin\/js\/children_drafts\.js \| \/children_drafts\.php, \/children\/drafts \| restored - legacy draft roster, bulk branch\/class modal, filters, exports, ACL, and bridge restored/,
);
assert.doesNotMatch(text.matrixMd, /children_drafts\.php[^\n]*live refresh audit remains/);

console.log("legacy children drafts roster contract verified.");
