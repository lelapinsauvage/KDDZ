...
const text = {
  legacyPhp: readFileSync(
    `${legacyRoot}/Front/templates/admin/Medical_forms2.php`,
    "utf8",
  ),
  legacyJs: readFileSync(
    `${legacyRoot}/Front/templates/admin/js/Medical_forms2.js`,
    "utf8",
  ),
  bridge: readFileSync("src/app/(app)/Medical_forms2.php/route.ts", "utf8"),
  page: readFileSync("src/app/(app)/medical/conditions/page.tsx", "utf8"),
  client: readFileSync(
    "src/app/(app)/medical/conditions/medical-conditions-client.tsx",
    "utf8",
  ),
...
assert.match(text.legacyPhp, /\$child = \$db->getChildinfoformtwo\(\)/);
...
assert.match(text.legacyPhp, /<h3 class="page-title">\s*Suffering Form\s*<\/h3>/);
assert.match(text.legacyPhp, /Suffering Form Listing/);
assert.match(text.legacyPhp, /id="newform1"[\s\S]*New Child Suffering Form/);
...
assert.match(text.legacyPhp, /js\/Medical_forms2\.js/);

assert.match(text.legacyJs, /\/\*var conn = new ab\.Session/);
assert.match(text.legacyJs, /conn\.subscribe\("new_form2"\+cat_master/);
...
assert.match(text.legacyJs, /getformtwoAllHashed/);
...
assert.match(text.legacyJs, /Medical_form2\.php\?id=/);
...
assert.match(text.bridge, /new URL\("\/medical\/conditions", request\.url\)/);
...
assert.match(text.guards, /legacyPage: "medical_reports\.php"[\s\S]*"\/Medical_forms2\.php"/);
assert.match(text.guards, /legacyPage: "medical_reports\.php"[\s\S]*prefixes: \["\/medical\/"\]/);

assert.match(text.page, /getMedicalForms\(\{ formType: "CONDITIONS", pageSize: "all" \}\)/);
...
assert.match(text.client, /title="Suffering Form"/);
assert.match(text.client, /Suffering Form Listing/);
assert.match(text.client, /New Child Suffering Form/);
assert.match(text.client, /filename: "suffering-medical-forms"/);
assert.match(text.client, /sheetName: "Suffering Form Listing"/);
...
assert.match(text.client, /href=\{`\/medical\/conditions\/\$\{row\.original\.id\}`\}/);
...
const row = matrix.find(
  (entry) => entry.legacyPhp === "Front/templates/admin/Medical_forms2.php",
);
assert.ok(row, "page parity row for Medical_forms2.php");
assert.equal(
  row.status,
  "restored - legacy suffering form roster, TableTools exports, print, visual smoke, and bridge restored",
);
...
assert.match(
  row.verification,
  /verify-legacy-suffering-medical-roster-contract\.ts/,
);
assert.match(
  text.matrixMd,
  /Medical_forms2\.php \| Front\/templates\/admin\/js\/Medical_forms2\.js \| \/Medical_forms2\.php, \/medical\/conditions \| restored - legacy suffering form roster, TableTools exports, print, visual smoke, and bridge restored/,
);
assert.doesNotMatch(text.matrixMd, /Medical_forms2\.php[^\n]*final logged-in visual smoke/);

console.log("legacy suffering medical roster contract verified.");
