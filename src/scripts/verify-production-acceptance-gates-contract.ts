...
assert.match(contents.gates, /P01-P07\/P10\/P12 cron gate coverage/);
assert.match(contents.gates, /P01-P03\/P05-P09\/P11-P15\/P17 provider gate coverage/);
assert.match(contents.gates, /P15-P17 native gate coverage/);
assert.match(contents.evidenceTemplate, /P01-P07\/P10\/P12 report id\/path/);
assert.match(contents.evidenceTemplate, /P01-P03\/P05-P09\/P11-P15\/P17 report id\/path/);
assert.match(contents.evidenceTemplate, /P15-P17 report id\/path/);
assert.match(contents.evidenceTemplate, /P17 report id\/path/);
assert.match(contents.cutoverRunbook, /P01-P07\/P10\/P12 cron coverage id\/path/);
assert.match(contents.cutoverRunbook, /P01-P03\/P05-P09\/P11-P15\/P17 provider coverage id\/path/);
assert.match(contents.cutoverRunbook, /P15-P17 native coverage id\/path/);
assert.match(contents.cutoverRunbook, /P17 notifications_nature coverage id\/path/);
...
assert.match(readinessAudit, /validateLocalEvidenceArtifact/);
assert.match(readinessAudit, /invalid local artifact/);
assert.match(readinessAudit, /genericReportEvidence/);
assert.match(readinessAudit, /PROVIDER_CHANNEL_DECISION_REPORT/);
...
assert.match(evidenceRecordVerifier, /--acceptance-date=<YYYY-MM-DD>/);
assert.match(evidenceRecordVerifier, /--require-zero-artifacts/);
assert.match(evidenceRecordVerifier, /Acceptance date must include/);
assert.match(evidenceRecordRenderer, /preflight release metadata/);
assert.match(evidenceRecordRenderer, /--acceptance-date/);
...
assert.match(partialReporter, /legacy-production-acceptance-gates\.md/);
assert.match(partialReporter, /page-parity-matrix\.json/);
assert.match(partialReporter, /--production-gates/);
assert.match(partialReporter, /validatePartialGateMap/);
assert.match(partialReporter, /references unknown production gate/);
...
assert.match(contents.evidenceTemplate, /Production preflight manifest/);
assert.match(contents.evidenceTemplate, /Production preflight manifest SHA-256/);
...
assert.match(contents.gates, /--require-zero-artifacts/);
assert.match(contents.gates, /final artifacts still contain blocking partial rows when `--require-zero-artifacts` is set/);
assert.match(contents.gates, /branch\/commit\/date is stale/);
assert.match(contents.gates, /rejects branch\/commit\/date drift from preflight release metadata/);
...
assert.match(contents.gates, /preflight readiness env template digests/);
...
assert.match(contents.gates, /local JSON file pointers for known migration\/file and production evidence artifacts are shape-checked/);
assert.match(contents.gates, /malformed local artifacts keep the gate in `needs-evidence`/);
...
assert.match(contents.gates, /readiness env template digests\/placeholders/);
...
assert.match(contents.gates, /preflight readiness env template artifacts/);
...
assert.match(contents.gates, /bind the generated work-order proof commands to the preflight `generatedAt` timestamp/);
...
assert.match(contents.gates, /unbound generatedAt placeholders in release-bound work orders/);
...
assert.match(contents.gates, /--preflight-manifest=<path>/);
...
assert.match(contents.gates, /--preflight-digest=<sha256>/);
...
assert.match(contents.gates, /`--generated-at=<iso>` with `--include-work-orders`/);
assert.match(contents.gates, /template records the exact release ref and timestamp beside the work-order comments/);
...
assert.match(contents.gates, /requiring every nested focused partial\/checklist artifact to match the manifest timestamp, source matrix\/gate-map\/production-gates paths, and gate filter/);
...
assert.match(contents.gates, /report-production-closeout-plan\.ts --json --out=\/tmp\/kiddzonl-production-closeout-plan\.json --generated-at=<release-generated-at-iso>/);
...
assert.match(contents.gates, /requiring the blocker-status report plus nested focused manifest and closeout plan to match the source matrix\/gate-map\/production-gates paths recorded in the saved manifest/);
assert.match(contents.gates, /requiring all bundled JSON artifacts to share the preflight manifest `generatedAt` timestamp/);
assert.match(contents.gates, /blockingGateSummary/);
assert.match(contents.gates, /source matrix\/gate-map\/production-gates paths without printing values/);
...
assert.match(contents.cutoverRunbook, /matching source matrix\/gate-map\/production-gates paths/);
...
assert.match(contents.cutoverRunbook, /preflight readiness env template hashes/);
assert.match(contents.cutoverRunbook, /archived redacted readiness report, closeout summary, partial gate report, production evidence checklist, and production preflight manifest/);
