...
| `new_assessment` | Assessment `_legacyNewAssessmentMarkers` / notification stub, including source database/key/table marker provenance |
...
| `t_holiday` | Holiday, including source database/key provenance and raw legacy row |
...
### Medical Forms
The legacy `t_medical_forms` catalogue used by `Data::getForms()` and `Data::getFormRef()` is preserved as `LegacyMedicalFormDefinition` rows with sourceDatabase, legacyKey/id, form name, ref target, active flag, created timestamp, and raw legacy data. The old child medical form tables are consolidated into `MedicalForm`. Form-specific fields remain in the `data` JSON, and active `t_form_1` through `t_form_5` rows also keep `sourceDatabase`, `legacyKey`, `legacyId`, `legacyTable`, child/branch/class/user legacy ids, and a raw row `legacyData` snapshot for reconciliation. The `t_med_forms_info` detail rows become `MedicalFormEntry` records with the same source/key provenance, legacy form and child ids, generated field/value text, and raw legacy row JSON. `t_form_4` also fans populated vaccine date/status pairs into `Vaccination` rows that keep the source form id, legacy child id, vaccine name, legacy date/status field names, and raw source row for row-level audit.

### Assessments
The seven legacy assessment tables are consolidated into `Assessment`. Answer keys (`m*`, `c*`, `l*`, `s*`, `d*`) stay as flat JSON keys so the modern assessment editor can reopen the migrated report. Active rows keep `sourceDatabase`, `legacyKey`, `legacyId`, `legacyTable`, child/class/teacher/user legacy ids, answer payload JSON, and raw legacy row JSON for reconciliation. Legacy `new_assessment` rows are preserved as source-provenanced markers on the matching assessment, or as a stub assessment with `new_assessment` provenance when the notification marker has no matching report row; reconciliation counts stub rows plus `_legacyNewAssessmentMarkers` entries for exact marker parity.
