...
for (const expected of [
  ...
  "focused artifact manifest, closeout plan, SHA-256 digests",
  "top-level `blockingGateSummary`",
  "rejects drift between `blockingGateSummary` and the nested blocker-only gate-status report",
  "preflight verifier now requires the blocker-status report plus nested focused manifest and closeout plan to match those recorded source paths",
  "requires the blocker-status report and closeout plan to retain verified source alignment",
  ...
  "The acceptance verifier can require `--preflight-manifest=<path>`, `--preflight-digest=<sha256>`, and `--require-zero-artifacts`",
  "rejects partial/checklist/preflight artifacts that still carry blocking rows",
  ...
  "verify-production-acceptance-evidence-record-contract.ts",
  "verify-production-acceptance-evidence-renderer-contract.ts",
  "final `--require-zero-partials` mode, evidence package verification now additionally requires the closeout partial summary, checklist blocking count, preflight blocking rows, preflight blocking gate links, `ready-for-final-closeout`, and `canCloseLocally: true`",
  "shape-checks local JSON evidence pointers for migration/file artifacts and production evidence artifacts",
  "malformed local JSON evidence keeps the gate in `needs-evidence`",
  "preflight manifest verifier rejects release metadata drift in those archived closeout-plan env-template commands",
]) {
