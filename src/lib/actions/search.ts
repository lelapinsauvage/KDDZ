...
export async function globalSearch(query: string): Promise<GlobalSearchResult> {
  ...
  return {
    children: children.map((c) => ({
      ...
      description: [c.class?.name, c.branch?.name].filter(Boolean).join(" · ") || "No class",
      href: `/children/${c.id}`,
    })),
    employees: [
      ...teachers.map((t) => ({
        ...
        description: ["Teacher", t.branch?.name].filter(Boolean).join(" · "),
      ...
      ...nurses.map((n) => ({
        ...
        description: ["Nurse", n.branch?.name].filter(Boolean).join(" · "),
      ...
      ...doctors.map((d) => ({
        ...
        description: ["Doctor", d.branch?.name].filter(Boolean).join(" · "),
      ...
      ...managers.map((m) => ({
        ...
        description: ["Manager", m.branch?.name].filter(Boolean).join(" · "),
